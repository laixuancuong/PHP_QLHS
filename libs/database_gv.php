...
$sql = "SELECT * FROM giaovien gv INNER JOIN chucnang_gv cngv ON gv.magv = cngv.magv INNER JOIN khoi k ON k.makhoi = cngv.makhoi INNER JOIN monhoc mh ON cngv.mamh = mh.mamh WHERE gv.magv = " . $_SESSION['user_gv'] . "
                        ";
$mon = $conn->query($sql);
$sql = "SELECT * FROM chucnang_gv cngv INNER JOIN phancong pc ON pc.id_cn_gv = cngv.id_cn_gv
                    INNER JOIN monhoc mh ON mh.mamh = cngv.mamh
...
$lop = $conn->query($sql);
$sql = "SELECT * FROM phancong_cn pccn INNER JOIN lop l ON l.malh = pccn.malh
        INNER JOIN phancong pc ON pc.id_pc_cn = pccn.id_pc_cn 
        INNER JOIN chucnang_gv cngv ON cngv.id_cn_gv = pc.id_cn_gv
        WHERE cngv.magv = " . $_SESSION['user_gv'] . "
                        ";
...
/*--Điểm--------------------*/
$sql = "SELECT * FROM chucnang_gv cngv INNER JOIN monhoc mh ON cngv.mamh = mh.mamh INNER JOIN diem_chitiet dct ON dct.mamh = mh.mamh  INNER JOIN hocsinh hs ON dct.mahs = hs.mahs
INNER JOIN diem d ON d.mahs = hs.mahs
